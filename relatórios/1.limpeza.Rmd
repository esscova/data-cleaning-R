...
O tratamento e limpeza de dados é uma etapa fundamental do processo de ciência de dados. Sem dados de qualidade, é impossível gerar insights confiáveis e tomar decisões informadas.
...
```{r}
dados <- read.csv("D:\\Projetos\\Data-science\\data cleaning R\\dados\\Churn.csv", sep = ";", na.strings="", stringsAsFactors=T)
```
...
Exploramos os dados em busca de inconsistências.
...
```{r}
# buscando registros (linhas) no dataset que tem valores.NAs
dados[!complete.cases(dados),]
```

## Tratando os dados

Após identificar inconsistências, iniciamos o tratamento dos dados.

-   Salários - Substituindo valores faltantes pela mediana
